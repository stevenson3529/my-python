...
title_image = pygame.image.load('assets/drop title.jpg')
game_over_image = pygame.image.load('assets/drop go.jpg')

windowWidth = 400
windowHeight = 600

surface = pygame.display.set_mode((windowWidth, windowHeight))
pygame.display.set_caption('drop - the official game')
...
def movePlayer():
    ...
    if surface.get_at((int(player['x']) + int(player['width']), int(player['y']) + int(player['height']))) == (0,0,0,255):
    ...
    if leftOfPlayerOnPlatform is False and rightOfPlayerOnPlatform is False and (player['y'] + player['height']) + player['vy'] < windowHeight:
        ...
        while foundPlatformTop is False:
            ...
            if surface.get_at((int(player['x']), int((player['y']) + int(player['height'])) - yOffset)) == (0,0,0,255):
                player['y'] -= yOffset
